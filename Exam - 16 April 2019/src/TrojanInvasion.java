...
public class TrojanInvasion {
    public static void main(String[] args) {
        ...
        Deque<Integer> defenders = Arrays.stream(scanner.nextLine().split("\\s+"))
        ...
        for (int i = 1; i <=waves && defenders.isEmpty() ; i++) {
            ...
            Arrays.stream(scanner.nextLine().split("\\s+"))
                    .mapToInt(Integer::parseInt)
                    .forEach(e-> attack.push(e));
            if(i%3==0){
                int additionalDefender=Integer.parseInt(scanner.nextLine());
                defenders.offer(additionalDefender);
            }
            while (!defenders.isEmpty() && !attack.isEmpty()){
                int attaker =attack.pop();
                int defender= defenders.poll();
                if(attaker>defender){
                    attaker-=defender;
                    attack.push(attaker);
                }else if(attaker<defender){
                    defender-=attaker;
                    defenders.offer(defender);
                }
            }
        }
        ...
        if(defenders.isEmpty()){
            ...
            System.out.printf("Warriors left: %s%n",
                    String.join(", ",attack.toString()).replaceAll("\\[|\\]",""));
        }else {
